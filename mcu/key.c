...
#define KEY_ONES_END_OF_SCAN 16
...
void key_update(void)
{
    ...
    uint8_t key_test = gpio_get(KEY_DATA_PIN) == KEY_SWITCH_CLOSED? 1 : 0;
    if (key_test)
    {
        key_ones++;
    ...
    if (key_ones == KEY_ONES_END_OF_SCAN)
    {
        key_index = 0;
    }
    else
...
{
    if (key_used[key_waiting] == KEY_USED_FALSE)
    {
        return key_waiting;
    }
    return 0;
...
uint8_t key_data_get(void)
{
    ...
    key_used[key_waiting] = KEY_USED_TRUE;
    key_waiting = 0;
    return key;
}
